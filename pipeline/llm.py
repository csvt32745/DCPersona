...
def build_llm_input(collected_messages, system_prompt_parts, cfg, discord_client_user_id, 
                   accept_usernames=False, datetime_now=None):
    """
    ...
    detailed_time = datetime_now.strftime("%Y-%m-%d %A %H:%M")
    ...
    final_messages = collected_messages.copy()
...
async def call_llm_api(openai_client, model_name, messages, tools=None, cfg=None):
    """
    ...
    Args:
        ...
        tools (list): List of tools available to the LLM
        cfg (dict): Configuration data
        
    Returns:
        dict: Response content and finish reason
    """
    if cfg is None:
        cfg = {}
        
    extra_api_parameters = cfg.get("extra_api_parameters", {})
